...
export function OperationCard({
  ...
  return (
    <Card className="group relative overflow-hidden border border-border/50 bg-background/50 backdrop-blur-sm transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 hover:border-primary/30">
      ...
      <div className={cn("absolute inset-0 bg-linear-to-br opacity-0 group-hover:opacity-100 transition-opacity duration-500", config.bg)}></div>
      ...
      <CardHeader className="relative z-10 pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className={cn("text-lg font-bold transition-colors group-hover:text-primary", config.text)}>
            {title}
          </CardTitle>
      ...
      <CardContent className="relative z-10">
        <div className="flex items-start justify-between gap-4">
          ...
          <Link href={`${href}?status=ready` as any} className="flex-1">
            ...
            >
              <span className="text-4xl font-extrabold mb-1">{stats.toProcess}</span>
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link
              ...
              className={cn(
                ...
                stats.late > 0 ? "bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/20" : "bg-muted/50 hover:bg-muted"
              )}
            >
              <AlertCircle className={cn("h-4 w-4", stats.late > 0 ? "text-orange-500" : "text-muted-foreground")} />
              <div className="flex flex-col items-start">
                <span className={cn("text-sm font-bold", stats.late > 0 ? "text-orange-600" : "text-muted-foreground")}>
                  {stats.late}
                </span>
      ...
      <div className={cn("absolute bottom-0 left-0 right-0 h-1 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left", `bg-linear-to-r ${config.gradient}`)}></div>
    </Card>
  );
}
